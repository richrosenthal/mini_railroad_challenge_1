...
#FUNCTION: Find the fastest path using networkx
def find_fastest_path(rail_graph, start_station, destination_station, verbose=False):
    '''
    Uses networkx's builtin Dijkstra implementation
    (nx.single_source_dijsktra) to find the fastest route.
    Returns the same dict shape as the hand-written version, so everything downstream (dispatch loop, table, CSV export)
    ...
    try:
        total_time, path = nx.single_source_dijkstra(
            rail_graph, source=start_station, target=destination_station, weight="weight"
        )
        reachable = True
        ...
        if verbose:
            print(f"No path exists between {start_station} and {destination_station}")
        ...
        if verbose:
            print(f"Station not found in graph: {start_station} -> {destination_station}")
    #Stop the dispath timer
...
for station in rail_graph.nodes:
    print(f"\nStation: {station}")

    for neighbor in rail_graph.neighbors(station):
        travel_time = rail_graph[station][neighbor]["weight"]
